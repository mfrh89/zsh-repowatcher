...
`REPOWATCHER_ROOTS=(~)` searches the home directory, with the same exclusions and depth limit. No personal paths are hardcoded. With no roots configured, only the current-repository prompt integration is active. Scanning is currently an explicit command, not a periodic background service.
