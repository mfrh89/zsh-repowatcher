...
The `precmd` hook starts a detached fetch worker; notifications and updates remain in the foreground. Cache keys derive from the canonical shared Git directory, so linked worktrees share a fetch interval and lock. Successful-fetch state is separate from attempt state: failures are throttled but do not qualify for automatic updates.
