...
The `chpwd` hook records transitions between canonical worktree roots. The `line-init` ZLE hook consumes a pending entry (including shell startup inside a repo), starts a detached fetch worker, and draws the normal prompt before any notice; notifications and updates remain in the foreground. Same-worktree directory changes retain pending completion callbacks. Ordinary prompts do not start workers. Cache keys derive from the canonical shared Git directory, so linked worktrees share a scan interval and lock, while entry detection remains worktree-specific. Successful-fetch state is separate from attempt state: scan failures are throttled but do not qualify for automatic updates. Entry fetches bypass the interval.
